...
| M2 | Kotlin 渲染（预算/compact） | JVM 单测覆盖预算截断/compact/ref_not_found；`.\gradlew :agent-browser-kotlin:test` exit=0 | `.\gradlew :agent-browser-kotlin:test` | todo |
| M3 | JS snapshot + Kotlin 解析 | `snapshotJs()` 生成可执行表达式；解析 snapshot JSON 成功；单测覆盖 JSON schema | `.\gradlew :agent-browser-kotlin:test` | todo |
| M4 | action(click/fill) 最小闭环 | `actionJs()` + `parseActionResult()` + `ref_not_found` 行为单测 | `.\gradlew :agent-browser-kotlin:test` | todo |
| M5 | Android E2E（WebView） | 在测试页完成 click/fill 导致 DOM 变化并被 re-snapshot 捕获（instrumentation test） | `.\gradlew :app:connectedAndroidTest` | done（见 `docs/evidence/2026-02-15-connectedAndroidTest.txt`） |
...
| REQ-0002-003 | v1-agent-browser-core §TDD-Render | `.\gradlew :agent-browser-kotlin:test` | — | todo |
...
| REQ-0002-007 | v1-agent-browser-core §TDD-SnapshotSchema | `.\gradlew :agent-browser-kotlin:test` | — | todo |
...
| REQ-0002-011 | v1-agent-browser-core §TDD-Action | `.\gradlew :agent-browser-kotlin:test` | — | todo |
| REQ-0002-012 | v1-agent-browser-core §TDD-Render | `.\gradlew :agent-browser-kotlin:test` | — | todo |
